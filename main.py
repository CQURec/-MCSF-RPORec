...
from DGI.dgi import DGI, Encoder
import evaluate
import time
...
class Model():
    ...
    def __init__(self, args, isLoad=False):
        ...
        if os.path.exists(iui_subGraph_data):
            ...
            _, self.iui_node_subGraph, self.iui_subGraph_adj, self.iui_dgi_node = buildSubGraph(self.metaPath['IUI'],
                                                                                                self.args.subNode)
        ...
        self.valid_loader = dataloader.DataLoader(valid_dataset, batch_size=args.test_batch * 101, shuffle=False,
                                                  num_workers=0)
    ...
    def prepareModel(self):
        ...
        # self.act = t.nn.ReLU()
        self.model = MODEL(self.uu_graph,self.ii_graph,self.out_dim ,len(self.user_graph), len(self.item_graph), self.args, self.userNum, self.itemNum,
                           self.hide_dim, \
    ...
    def run(self):
        ...
        for e in range(self.curEpoch, self.args.epochs + 1):
            ...
            log("**************************************************************")
            log("start train")
            epoch_loss, epoch_uu_dgi_loss, epoch_ii_dgi_loss, epoch_uiu_dgi_loss, epoch_uitiu_dgi_loss, epoch_iui_dgi_loss = self.trainModel(is_print)
            log("end train")
            self.train_loss.append(epoch_loss)
            log("epoch %d/%d, epoch_loss=%.2f, dgi_uu_loss=%.4f, dgi_ii_loss=%.4f, dgi_uiu_loss=%.4f, dgi_uitiu_loss=%.4f, dgi_iui_loss=%.4f" % \
                (e, self.args.epochs, epoch_loss, epoch_uu_dgi_loss, epoch_ii_dgi_loss, epoch_uiu_dgi_loss,
                 epoch_uitiu_dgi_loss, epoch_iui_dgi_loss))
    ...
    def test(self):
        ...
        self.test_loader = dataloader.DataLoader(test_dataset, batch_size=args.test_batch * 101, shuffle=False,
                                                 num_workers=0)
        HR, NDCG = self.validModel(self.test_loader)
        return HR, NDCG

    def trainModel(self, is_print):
        ...
        for user, item_i, item_j in train_loader:
            ...
            user_embed, item_embed, uu_s, ii_s = self.model(self.uv_g, self.user_graph, self.item_graph, self.time_seq_tensor,
                                                self.uu_subGraph_adj_tensor, self.uu_subGraph_adj_norm, self.uu_node_subGraph,
                                                self.ii_subGraph_adj_tensor, self.ii_subGraph_adj_norm, self.ii_node_subGraph,
                                                self.ratingClass,True)

            # u_embeddings = user_embed.detach().cpu().numpy()  # 转换为numpy数组
            #
            # # 使用t-SNE进行降维
            # tsne = TSNE(n_components=2, random_state=0)
            # embeddings_2d = tsne.fit_transform(u_embeddings)
            #
            # # 可视化
            # plt.figure(figsize=(8, 8))
            # plt.scatter(embeddings_2d[:, 0], embeddings_2d[:, 1], c='b', alpha=0.5)
            # plt.show()
            # exit()
            ...
            # if is_print:
            #     self.print_emb(userEmbed, posEmbed, negEmbed)
            #     exit(0)
            # 使用t-SNE进行降维可视化（用户和物品）
            ...
            ssl_uu_loss = self.ssl_loss(user_embed, uu_s, user)
            ssl_ii_loss = self.ssl_loss(item_embed, ii_s, item_i)
    ...
    def print_emb(self, userEmbed, posEmbed, negEmbed):
        tsne = TSNE(n_components=2, random_state=42)

        # 对用户嵌入和物品嵌入进行降维
        user_embeddings_tsne = tsne.fit_transform(userEmbed.cpu().detach().numpy())
        item_embeddings_tsne = tsne.fit_transform(posEmbed.cpu().detach().numpy())
        neg_item_embeddings_tsne = tsne.fit_transform(negEmbed.cpu().detach().numpy())
        # 绘制用户嵌入的散点图
        plt.scatter(user_embeddings_tsne[:, 0], user_embeddings_tsne[:, 1], c='blue', label='user')

        # 绘制物品嵌入的散点图
        plt.scatter(item_embeddings_tsne[:, 0], item_embeddings_tsne[:, 1], c='red', label='pos_item')

        plt.scatter(neg_item_embeddings_tsne[:, 0], neg_item_embeddings_tsne[:, 1], c='yellow', label='neg_item')
        # 添加图例
        plt.legend()
        plt.savefig('mdif-all.pdf', format='pdf')
        # 显示图像
        plt.show()
    ...
    def validModel(self, data_loader, save=False):
        HR, NDCG = [], []
        data = {}
        user_embed, item_embed= self.model(self.uv_g, self.user_graph, self.item_graph, self.time_seq_tensor,
                                                self.uu_subGraph_adj_tensor, self.uu_subGraph_adj_norm, self.uu_node_subGraph,
                                                self.ii_subGraph_adj_tensor, self.ii_subGraph_adj_norm, self.ii_node_subGraph,
                                                self.ratingClass,False)
