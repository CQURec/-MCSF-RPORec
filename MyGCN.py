...
class MODEL(nn.Module):
    def __init__(self,uu_graph, ii_graph, out_dim, user_graph_len, item_graph_len, args, userNum, itemNum, hide_dim, maxTime, interactionNum=5,
                 layer=[16, 16]):
        super(MODEL, self).__init__()
        self.encoder_act = nn.Tanh()
        self.out_dim = out_dim
        self.uu_encoder = Encoder(uu_graph, self.out_dim, self.out_dim, nn.PReLU()).cuda()
        self.ii_encoder = Encoder(ii_graph, self.out_dim, self.out_dim, nn.PReLU()).cuda()
    ...
    def forward(self, graph, usergraph, itemgraph, time_seq,  uu_subGraph_adj_tensor, uu_subGraph_adj_norm, uu_node_subGraph,
                                                ii_subGraph_adj_tensor, ii_subGraph_adj_norm, ii_node_subGraph, rClass=5, isTrain=True):
        norm = 1
        ...
        all_user_embeddings_cl = [self.embedding_dict['user_emb']]
        all_item_embeddings_cl = [self.embedding_dict['item_emb']]
        if len(self.layers) == 0:
            item_embedding = self.embedding_dict['item_emb'].view(-1, rClass, self.out_dim)
        ...
        all_embeddings_cl = t.cat([self.embedding_dict['user_emb'], self.embedding_dict['item_emb']], 0)
        for i, layer in enumerate(self.layers):
            if i == 0:
                embeddings = layer(graph, self.embedding_dict['user_emb'], self.embedding_dict['item_emb'], edge_feat)
                embeddings_cl = embeddings
            else:
                embeddings = layer(graph, embeddings[: self.userNum], embeddings[self.userNum:], edge_feat)
                embeddings_cl = layer(graph, embeddings_cl[: self.userNum], embeddings[self.userNum:], edge_feat)

            if isTrain:
                random_noise = t.rand_like(embeddings).cuda()
                embeddings += t.sign(embeddings) * F.normalize(random_noise, dim=-1) * 0.2
            ...
            norm_embeddings_cl = F.normalize(embeddings_cl, p=2, dim=1)
            all_user_embeddings_cl += [norm_embeddings_cl[: self.userNum]]
            all_item_embeddings_cl += [norm_embeddings_cl[self.userNum:]]
        user_embedding = t.cat(all_user_embeddings, 1)
        item_embedding = t.cat(all_item_embeddings, 1)
        user_embedding_cl = t.cat(all_user_embeddings, 1)
        item_embedding_cl = t.cat(all_item_embeddings, 1)

        if rClass == 1:
            return user_embedding, item_embedding
        item_embedding = item_embedding.view(-1, rClass, self.out_dim)
        item_embedding_cl = item_embedding_cl.view(-1, rClass, self.out_dim)
        if self.args.fuse == "mean":
            ret_item_embedding = t.div(t.sum(item_embedding, dim=1), rClass)
            item_embedding_cl = t.div(t.sum(item_embedding_cl, dim=1), rClass)
        ...
        if isTrain:
            user_random_noise = t.rand_like(user_embedding).cuda()
            user_embedding += t.sign(user_embedding) * F.normalize(user_random_noise, dim=-1) * 0.1
            item_random_noise = t.rand_like(ret_item_embedding).cuda()
            ret_item_embedding += t.sign(ret_item_embedding) * F.normalize(item_random_noise, dim=-1) * 0.1
            uu_emb = self.uu_encoder(user_embedding, corrupt = False)
            uu_sub_emb = t.sparse.mm(uu_subGraph_adj_tensor, uu_emb) / uu_subGraph_adj_norm
            uu_sub_emb = self.encoder_act(uu_sub_emb)
            uu_summary = uu_sub_emb[uu_node_subGraph]
            ii_emb = self.ii_encoder(ret_item_embedding, corrupt = False)
            ii_sub_emb = t.sparse.mm(ii_subGraph_adj_tensor, ii_emb) / ii_subGraph_adj_norm
            ii_sub_emb = self.encoder_act(ii_sub_emb)
            ii_summary = ii_sub_emb[ii_node_subGraph]

            # return (1-l_) * user_embedding + l_ * uu_summary, (1 - l_) * ret_item_embedding + l_ * ii_summary, uu_summary, ii_summary
            return  user_embedding, ret_item_embedding, user_embedding_cl, item_embedding_cl
        else:
            return user_embedding, ret_item_embedding
        self.semanticUserEmbeddings.append(user_embedding)
        self.semanticUserEmbeddings.append(uu_summary)
        self.semanticUserEmbeddings = t.stack(self.semanticUserEmbeddings, dim=1)
        self.semanticItemEmbeddings.append(ret_item_embedding)
        self.semanticItemEmbeddings.append(ii_summary)
        self.semanticItemEmbeddings = t.stack(self.semanticItemEmbeddings, dim=1)
        user_embedding = self.semanticUserAttention(self.semanticUserEmbeddings)
        ret_item_embedding = self.semanticItemAttention(self.semanticItemEmbeddings)
        return user_embedding, ret_item_embedding, uu_summary, ii_summary
...
class TimeEncoding(nn.Module):
    ...
    # 调用方法
    # 1.先初始化 t_e=TimeEncoding(隐藏层)#这个隐藏层随便定义，一般的话是和你原本模型的相同
